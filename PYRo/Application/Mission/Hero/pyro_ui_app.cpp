...
{
    // ui_ptr
    //     ->draw_circle("C01", pyro::ui_operate::ADD, 1, pyro::ui_color::GREEN, 2,
    //                   960, 540, 100)
    //     // 准星竖线
    //     .draw_line("L01", pyro::ui_operate::ADD, 1, pyro::ui_color::GREEN, 2,
    //                960, 540, 960, 600);
    // 1. 绘制静态文本标签 (注意名字不能重复)
    ui_ptr->draw_string("ST1", pyro::ui_operate::ADD, 3, pyro::ui_color::PINK, 20, 2, 110, 800, "FRIC1:");
    vTaskDelay(pdMS_TO_TICKS(50));
    ui_ptr->draw_string("ST2", pyro::ui_operate::ADD, 3, pyro::ui_color::PINK, 20, 2, 110, 750, "FRIC2:");
    vTaskDelay(pdMS_TO_TICKS(50));

    // 2. 为动态数值提前进行 ADD 占位，赋予初始值，方便后续直接 MODIFY
    ui_ptr->draw_float("DF1", pyro::ui_operate::ADD, 4, pyro::ui_color::WHITE, 20, 2, 230, 800, 0.0f)
          .draw_float("DF2", pyro::ui_operate::ADD, 4, pyro::ui_color::WHITE, 20, 2, 230, 750, 0.0f);
    ui_ptr->flush(); // 拼包发送
...
void ui_update_dynamic()
{
    ...
    if (speed_control_en)
    {
        ui_ptr->draw_float("DF1", pyro::ui_operate::MODIFY, 4, pyro::ui_color::YELLOW, 20, 2, 230, 800, fric1_mps)
          .draw_float("DF2", pyro::ui_operate::MODIFY, 4, pyro::ui_color::YELLOW, 20, 2, 230, 750, fric2_mps);
    }
    else
    {
        ui_ptr->draw_float("DF1", pyro::ui_operate::MODIFY, 4, pyro::ui_color::WHITE, 20, 2, 230, 800, fric1_mps)
          .draw_float("DF2", pyro::ui_operate::MODIFY, 4, pyro::ui_color::WHITE, 20, 2, 230, 750, fric2_mps);
    }
...
void uirxbooster()
{
    ...
    if (pyro::can_rx_drv_t::get_data(pyro::can_hub_t::which_can::can2, 0x110, raw_data))
    {
